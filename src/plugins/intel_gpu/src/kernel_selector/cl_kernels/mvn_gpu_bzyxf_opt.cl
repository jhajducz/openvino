...
typedef union _INPUT0_VECTOR_TYPE_UNION{
    INPUT0_TYPE f[F_BLOCK_SIZE];
    INPUT0_VECTOR_TYPE v;
} INPUT0_VECTOR_TYPE_UNION;

typedef union _ACCUMULATOR_VECTOR_TYPE_UNION{
    ACCUMULATOR_TYPE f[F_BLOCK_SIZE];
    ACCUMULATOR_VECTOR_TYPE v;
} ACCUMULATOR_VECTOR_TYPE_UNION;

typedef union ACTIVATION_FUNC_VECTOR_TYPE_UNION{
    ACTIVATION_FUNC_TYPE f[F_BLOCK_SIZE];
    ACTIVATION_FUNC_VECTOR_TYPE v;
} ACTIVATION_FUNC_VECTOR_TYPE_UNION;
...
ACCUMULATOR_TYPE inv_sqrt_var = (ACCUMULATOR_TYPE)0;
	...
	for (int fb = 0; fb < NUM_F_BLOCKS_PER_WI; fb++) {
		for (int i = 0; i<F_BLOCK_SIZE; i++) {
			in_vals_mean[fb].f[i] = work_group_reduce_add(in_vals_mean[fb].f[i]) / DATA_SET_SIZE;
#if NORMALIZE_VARIANCE == 1
			in_vals_squared_mean[fb].f[i] = work_group_reduce_add(in_vals_squared_mean[fb].f[i]) / DATA_SET_SIZE;
#endif
		}
	}
	*/
